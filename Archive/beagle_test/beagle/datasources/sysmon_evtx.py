...
class SysmonEVTX(WinEVTX):
    ...
    def parse_record(self, record: etree.ElementTree, name="") -> dict:
        ...
        for node in record:
            ...
            for child in node:
                if parent == "EventData" and "Name" in child.attrib:
        ...
        #     out["TimeCreated_SystemTime"], "%Y-%m-%d %H:%M:%S.%f")
        time = parser.parse(out["TimeCreated_SystemTime"])

        # int(time.strftime("%S"))
        out["TimeCreated_SystemTime"] = time.timestamp()
        return out
