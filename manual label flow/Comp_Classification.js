...
function Comp_Classification() {
  ...
  const [historicalCases, setHistoricalCases] = React.useState("");
  ...
  const get_allhistricalcase = "http://localhost:3004/get_allhistricalcase";
  ...
  useEffect(() => {
    fetch(get_classification_result_apiurl)
      ...
      .then((databody) => {
        ...
        if (databody.message.length > 1) {
          localStorage.setItem(
            "LastClassificCrimeType",
    ...
    fetch(get_allhistricalcase)
      ...
      .then((databody) => {
        console.log(databody.message);
        setHistoricalCases(
          databody.message.map(function (el) {
            return el.CrimeType;
          })
        );
      })
      .catch((error) => console.log(error));
    console.log(localStorage.getItem("historical_data"));
  }, []);

  console.log(historicalCases);
  const uniquehistoricalsamples = [...new Set(historicalCases)]; //check whether sample has all n crimetypes labels
  console.log(uniquehistoricalsamples);
  ...
  return (
    <div>
      ...
      {(() => {
        ...
        if (uniquehistoricalsamples.length < 3)
          return (
            <div>
              Insufficient historical data, cannot run classification. Please
              provide a manual correction.
              <Button style={{ margin: "10px" }} onClick={onClickManualLabel}>
                Manual Correction
              </Button>
            </div>
          );
          ...
          return (
            <div>
              No classification found, click on Run Classification button to
              start.
              <Button style={{ margin: "10px" }} onClick={runpredict}>
                Run Classification
              </Button>
